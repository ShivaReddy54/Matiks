...
exports.forget_password = async (req, res) => {
    try{
        ...
        // TODO -> generate the 10m reset JWT token and send it through the email and hit reset-pass route
        return res.status(200).json({ message: "Reset Token sent to mail"});
    }
    catch(err){
        return res.status(400).json({ error: err});
    }
...
exports.reset_password = async (req, res) => {
    try{
        ...
        // TODO -> Instead of email take the reset JWT and verify its expiry
        const salt = await bcrypt.genSalt(10);
        const hashed_password = await bcrypt.hash(password, salt);

        const response = await User.findOneAndUpdate({ email: email}, { $set: { password: hashed_password } }, { new: true, runValidators: true });
        if(!response) return res.status(400).json({ error: "Error resetting the user password"});
